...
export const MultipleImageUpload = forwardRef<MultipleImageUploadRef, MultipleImageUploadProps>(
  function MultipleImageUpload({ images: externalImages = [], onImagesChange, variant = 'journal' }, ref) {
    ...
    if (variant === 'neofeed') {
      return (
        <div className="w-full h-full p-1.5 flex flex-col bg-transparent relative overflow-hidden">
          <div className="flex-1 overflow-x-auto overflow-y-hidden flex flex-row gap-2 pb-1.5 custom-scrollbar snap-x snap-mandatory">
            {images.map((img, idx) => (
              <div key={img.id} className="group relative rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm transition-all hover:shadow-md flex-shrink-0 w-36 snap-start">
                <div className="aspect-video w-full overflow-hidden bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
                  ...
                  <div className="absolute top-1 right-1 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Button 
                      ...
                      size="icon" 
                      variant="secondary" 
                      className="h-5 w-5 rounded-full bg-white/90 dark:bg-gray-800/90 shadow-sm"
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                ...
                {imageCaptions[img.id] ? (
                  <div className="p-1 px-1.5 bg-gray-50/80 dark:bg-gray-800/80 backdrop-blur-sm border-t border-gray-100 dark:border-gray-700">
                    <p className="text-[9px] text-gray-600 dark:text-gray-400 truncate">{imageCaptions[img.id]}</p>
                  </div>
                ) : (
                  <button 
                    type="button"
                    ...
                    className="w-full p-1 text-center text-[8px] text-gray-400 hover:text-blue-500 transition-colors border-t border-gray-100 dark:border-gray-700"
                  >
                    Add caption...
                  </button>
                )}
                {editingImageId === img.id && (
                  <div className="absolute inset-0 bg-white/95 dark:bg-gray-900/95 flex flex-col p-1.5 animate-in fade-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
                    <div className="flex justify-between items-center mb-0.5">
                      <span className="text-[9px] font-medium text-gray-700 dark:text-gray-300">Edit Caption</span>
                      <Button type="button" size="icon" variant="ghost" className="h-4 w-4" onClick={() => setEditingImageId(null)}>
                        <X className="h-2.5 w-2.5" />
                      </Button>
                    </div>
                    <textarea
                      autoFocus
                      className="flex-1 bg-transparent border-0 text-[10px] resize-none focus:ring-0 text-gray-900 dark:text-white leading-tight"
                    ...
                    />
                    <Button type="button" size="sm" className="mt-0.5 w-full h-5 text-[9px]" onClick={() => setEditingImageId(null)}>Done</Button>
                  </div>
                )}
              </div>
            ))}
            ...
            {images.length < 5 && (
              <button
                ...
                className="flex-shrink-0 w-36 aspect-video rounded-lg border-2 border-dashed border-gray-200 dark:border-gray-700 hover:border-blue-400 dark:hover:border-blue-500 flex flex-col items-center justify-center gap-0.5 text-gray-400 hover:text-blue-500 transition-all bg-gray-50/50 dark:bg-gray-900/20 snap-start"
              >
                <div className="w-6 h-6 rounded-full bg-white dark:bg-gray-800 shadow-sm flex items-center justify-center border border-gray-100 dark:border-gray-700">
                  <Plus className="h-3 w-3" />
                </div>
                <div className="text-center">
                  <p className="text-[10px] font-medium">Add Image</p>
                  <p className="text-[8px] opacity-70">Up to 5 allowed</p>
                </div>
              </button>
            )}
          </div>
