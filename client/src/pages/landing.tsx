          ...
          <div className={`${showAccessInfo ? 'h-0 opacity-0 pointer-events-none' : 'h-auto opacity-100'} w-full flex justify-center p-0 m-0 transition-all duration-700 ease-in-out transform ${showAccessInfo ? 'translate-y-4 scale-95' : 'translate-y-0 scale-100'}`}>
            <div className="w-[280px] h-[160px] bg-gray-900/80 rounded-lg border border-gray-800 shadow-2xl relative overflow-hidden group">
              ...
              <div className="p-2 space-y-1 relative h-full flex flex-col">
                ...
                {/* Journal Tab Carousel Overlay */}
                <div className={`absolute inset-0 bg-gradient-to-b from-gray-100 to-gray-200 rounded-lg transition-all duration-500 ${showJournalCarousel && !showPerformanceWindow ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
                  <div className="h-full flex flex-col items-center justify-center relative p-3">
                    {/* Curved Stacked Cards */}
                    <div className="relative w-full h-24 flex items-center justify-center">
                      {carouselSlides.map((slide, idx) => {
                        ...
                        return (
                          <div
                            key={idx}
                            className={`absolute w-36 h-20 bg-white rounded-xl shadow-lg border border-gray-200 flex flex-col items-center justify-center transition-all duration-300`}
                            style={{
                              transform: `translateX(${offset * 20}px) scale(${1 - absOffset * 0.08}) rotateY(${offset * -5}deg)`,
                              zIndex: 10 - absOffset,
                              opacity: absOffset > 2 ? 0 : 1 - absOffset * 0.25,
                            }}
                          >
                            {slide.icon === "chart" && (
                              <svg className="w-5 h-5 text-purple-500 mb-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                <path d="M3 3v18h18M7 16l4-4 4 4 6-6" />
                              </svg>
                            )}
                            {slide.icon === "news" && (
                              <svg className="w-5 h-5 text-blue-500 mb-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                <rect x="3" y="3" width="18" height="18" rx="2" />
                                <path d="M7 7h10M7 11h10M7 15h6" />
                              </svg>
                            )}
                            {slide.icon === "tech" && (
                              <svg className="w-5 h-5 text-green-500 mb-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" />
                              </svg>
                            )}
                            {slide.icon === "journal" && (
                              <svg className="w-5 h-5 text-orange-500 mb-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                <path d="M4 19.5A2.5 2.5 0 016.5 17H20M4 4.5A2.5 2.5 0 016.5 2H20v20H6.5A2.5 2.5 0 014 19.5v-15z" />
                              </svg>
                            )}
                            {slide.icon === "stats" && (
                              <svg className="w-5 h-5 text-red-500 mb-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                <path d="M18 20V10M12 20V4M6 20v-6" />
                              </svg>
                            )}
                            <span className={`text-[8px] font-bold ${isActive ? 'text-gray-800' : 'text-gray-400'}`}>{slide.title}</span>
                            <span className="text-[6px] text-gray-400">{slide.subtitle}</span>
                          </div>
                        );
                      })}
                    </div>
                    ...
                    <svg className="w-full h-4 mt-2" viewBox="0 0 200 20" fill="none">
                      <path d="M 0 10 Q 100 25 200 10" stroke="#d1d5db" strokeWidth="1" fill="none" />
                    </svg>

                    {/* Indicator */}
                    <div className="bg-gray-600 text-white text-[6px] px-2 py-0.5 rounded-full font-medium">
            ...
            {isSignupVerification ? (
              ...
              <>
                ...
                <div className="space-y-3">
                  ...
                  >
                    {isLogin ? "Login" : "Sign Up"} <ArrowRight size={16} className="ml-2" />
                  </Button>
